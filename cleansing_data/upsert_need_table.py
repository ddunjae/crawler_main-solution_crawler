...
with SSHTunnelForwarder(
        ...
        df = pd.read_excel(base_url+"09_bu2.xlsx")
        ...
        query = f"SELECT id FROM need_cleansing_data WHERE id IN ({id_place_holders})"
        ...
        if len(ids_be_update) > 0:
            ...
            for i, row in pd_be_update.iterrows():
                ...
                query = f"UPDATE need_cleansing_data SET {set_values_str} WHERE id = {auction_id}"
        ...
        if len(ids_be_insert) > 0:
            ...
            for i, row in pd_be_insert.iterrows():
                ...
                query = f"INSERT INTO need_cleansing_data ({insert_columns}) VALUES ({values_str})"
                cursor.execute(query)
