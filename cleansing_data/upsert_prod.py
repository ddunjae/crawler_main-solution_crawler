...
with SSHTunnelForwarder(
        ...
        base_url = "/Users/hanseungjae/auction_solution/data_cleansing/0913/"
        df = pd.read_excel(base_url+"위트니.xlsx")
        ...
        query = f"SELECT id FROM prod_auction_data WHERE id IN ({id_place_holders})"
        ...
        if len(ids_be_update) > 0:
            ...
            for i, row in pd_be_update.iterrows():
                ...
                query = f"UPDATE prod_auction_data SET {set_values_str} WHERE id = {auction_id}"
        ...
        if len(ids_be_insert) > 0:
            ...
            for i, row in pd_be_insert.iterrows():
                ...
                query = f"INSERT INTO prod_auction_data ({insert_columns}) VALUES ({values_str})"
                cursor.execute(query)
