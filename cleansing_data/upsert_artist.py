...
with SSHTunnelForwarder(
        ...
        base_url = "/Users/hanseungjae/auction_solution/data_cleansing/1006/"
        df = pd.read_excel(base_url+"artist_new.xlsx")

        # define column in database
        db_columns = ["id","artist_id","name_kr", "name_en", "name_cn", "born", "dead", "nationality1", "nationality2","reference", "exhibition", "education", "description", "memo", "traditional","generation","art_movmnt"]
        ...
        query = f"SELECT id FROM artist WHERE id IN ({id_place_holders})"
        ...
        if len(ids_be_update) > 0:
            ...
            for i, row in pd_be_update.iterrows():
                ...
                query = f"UPDATE artist SET {set_values_str} WHERE id = {auction_id}"
        ...
        if len(ids_be_insert) > 0:
            ...
            for i, row in pd_be_insert.iterrows():
                ...
                query = f"INSERT INTO artist ({insert_columns}) VALUES ({values_str})"
                cursor.execute(query)
            
            cursor.execute("COMMIT;")

#artist_matching_data