...
with SSHTunnelForwarder(
        ...
        df = pd.read_excel(base_url+"윤형근.xlsx")
        ...
        if len(ids_be_update) > 0:
            ...
            for i, row in pd_be_update.iterrows():
                ...
                query = f"UPDATE crawling SET {set_values_str} WHERE id = {auction_id}"
        ...
        if len(ids_be_insert) > 0:
            ...
            for i, row in pd_be_insert.iterrows():
                ...
                query = f"INSERT INTO crawling ({insert_columns}) VALUES ({values_str})"
                cursor.execute(query)
